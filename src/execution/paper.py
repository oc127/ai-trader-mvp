...
class PaperExecutor(Executor):
    def __init__(
        self, client: HLRestClient, store: DataStore, initial_balance: float = 10000.0
    ) -> None:
        self._client = client
        self._store = store
    ...
    def execute_signal(self, signal: Signal) -> list[OrderResult]:
        ...
        for order_req in signal.orders:
            ...
            if order_req.side == Side.BUY and not order_req.is_spot:
                pass
            elif order_req.side == Side.BUY and order_req.is_spot:
            ...
            if order_req.side == Side.SELL and order_req.is_spot:
                self._balance += notional - fee

            self._update_paper_position(
                order_req.coin, order_req.side, order_req.size, fill_price, order_req.is_spot
            )
    ...
    def apply_funding(self, coin: str, rate: float) -> float:
        ...
        log.debug(
            "Paper funding applied", extra={"coin": coin, "rate": rate, "payment": funding_payment}
        )
        return funding_payment

    def _update_paper_position(
        self, coin: str, side: Side, size: float, price: float, is_spot: bool
    ) -> None:
