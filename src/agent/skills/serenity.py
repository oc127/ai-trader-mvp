...
每条支撑论点的信息，必须标注证据等级：
- **Strong**: 交易所公告(SEC/HKEX/SSE等)、年报/季报、财报电话会纪要、官方合同/订单/产能预留、专利/认证、监管审批
- **Medium**: 权威财经媒体、行业协会数据、卖方研报(假设可见)、供应商/客户交叉验证、公司官网产品页
- **Weak**: KOL帖子、社交媒体、论坛、来源不明截图、无基本面支撑的量价异动
- **Needs checking**: 重要但尚未核实

规则：不能仅靠 Weak 级证据把候选标的排到前列。
...
## 惩罚因子（每项 0-5 分，扣分 = 评级 × 2）

逐项检查，有问题就扣分，没问题给 0：

| 惩罚项 | 评级 | 扣分 |
|--------|------|------|
| 稀释/融资风险：需要融资才能撑到放量吗？ | ?/5 | ? |
| 治理风险：管理层历史、关联交易、信披质量 | ?/5 | ? |
| 地缘风险：制裁/出口管制/断供风险 | ?/5 | ? |
| 流动性风险：日均成交额、机构可建仓深度 | ?/5 | ? |
| 炒作风险：是否已被社交媒体/KOL热炒 | ?/5 | ? |
| 周期性风险：需求是结构性还是周期性 | ?/5 | ? |
| 替代设计风险：技术路线被绕过的概率 | ?/5 | ? |
